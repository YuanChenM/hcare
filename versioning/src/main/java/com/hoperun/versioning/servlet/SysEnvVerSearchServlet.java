 ...
 */
@WebServlet(name="SysEnvVerSearchServlet",urlPatterns="/SysEnvVerSearch")
public class SysEnvVerSearchServlet extends HttpServlet {

    private static final long serialVersionUID = 1L;
    private final static Logger logger = LoggerFactory.getLogger(SysEnvVerSearchServlet.class);

    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        logger.info("doPost");

        String sysEnvName = String.valueOf(request.getParameter("sysEnvName"));
        String svnVersion = String.valueOf(request.getParameter("svnVersion"));
        String isSteady = String.valueOf(request.getParameter("isSteady"));
        String isRuning = String.valueOf(request.getParameter("isRuning"));

        RuningVerHistoryService runingVerHistoryService = new RuningVerHistoryService();
        try{
            List<RuningVersionEntity> runVerHistoryList = runingVerHistoryService.searchRunVerHistory(sysEnvName,svnVersion,isSteady,isRuning);
